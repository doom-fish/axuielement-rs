...
This audit enumerates the public surface of the HIServices AXUIElement framework (via `AXUIElement.h` and `AXValue.h` headers in MacOSX26.2.sdk) and cross-references every symbol against the crate's safe Rust API (`src/**/*.rs`) and raw FFI bindings (`src/ffi/` when `raw-ffi` feature is enabled). All 65 SDK public symbols have been verified: 62 are reachable through the safe interface or the default `raw-ffi` declarations, and 3 are deprecated APIs excluded from coverage per v2 audit rules (marked with `CF_DEPRECATED_MAC(10_x, 10_9)` in the SDK headers). The 100% figure is for those 65 symbols of MacOSX26.2.sdk only: `AXWebConstants.h` (161 web-content names, 35 of them text-marker attributes, present in the macOS 26 and 27 SDKs) is not counted and not wrapped, and 12 of the VERIFIED rows (the `AXValueType` enum and legacy constants) are raw `ffi` declarations that the safe API models as `AXValueKind`.
